...
import qualified Texture (lexer)
import Yi
...
import Yi.Buffer.Misc (modeAlwaysApplies)
...
textureMode :: TokenBasedMode StyleName
textureMode = styleMode Texture.lexer
  & modeNameA           .~ "texture"
  & modeAppliesA        .~ modeAlwaysApplies
  & modeIndentSettingsA .~ IndentSettings
    { expandTabs = False
    , tabSize    = 2
    , shiftWidth = 2
    }